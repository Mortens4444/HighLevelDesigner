...
namespace Mtf.Core
{
	public sealed partial class ErrorBox : BaseBox
	{
        ...
        private void SetPinButtonVisible(bool visible)
        {
            if (!infinite)
            {
                btn_Pin.Visible = visible;
                btn_Unpin.Visible = !visible;
            }
        }
	    ...
	    private void ShowMessageOnOkButton()
		{
			...
			okSecondsLeft.Append($" ({SecondsLeft})");
	    ...
	    private void SaveMessage()
	    {
	        ...
	        var folder = $"{Application.ProductName} - {Application.ProductVersion}";
